...
- **`sw.js`**: cache-first service worker. `CACHE_NAME` is versioned (`account-book-v4`). **Bump `CACHE_NAME` whenever you change `index.html`, `manifest.json`, or icons** — otherwise installed users keep the stale cached version (SW also runtime-caches any same-origin asset on first fetch).
...
- Version string (currently "v1.3") appears in the About modal and the onload `console.log` — keep both in sync.
